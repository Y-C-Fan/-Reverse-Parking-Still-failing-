...
const SteeringWheel: React.FC<SteeringWheelProps> = ({ angle, maxAngle, onChange }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2 select-none">
        {/* Status Text */}
        <div className="text-center font-mono text-sm mb-1">
        ...
        <div className="relative">
            <div 
                ref={wheelRef}
                className={`w-40 h-40 md:w-48 md:h-48 rounded-full border-[8px] shadow-2xl relative touch-none transition-transform duration-75 ease-out
            ...
            >
                ...
                {/* Center Hub */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-gray-700 rounded-full shadow-inner flex items-center justify-center z-10">
                    <div className="w-12 h-12 rounded-full bg-gray-800 border border-gray-600 flex items-center justify-center">
                        <div className="w-8 h-1.5 bg-blue-600/50 rounded-full"></div>
                    </div>
                </div>

                {/* Spokes */}
                <div className="absolute top-1/2 left-0 w-full h-4 bg-gray-700 -translate-y-1/2 rounded-full"></div>
                <div className="absolute top-1/2 left-1/2 w-4 h-1/2 bg-gray-700 -translate-x-1/2 rounded-b-lg origin-top"></div>

                {/* Top Dead Center Marker (Tape) */}
                <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-4 h-8 rounded-b shadow-md z-20 transition-colors
                    ${Math.abs(Number(displayTurns)) < 0.1 ? 'bg-blue-500' : 'bg-yellow-500'}
                `}></div>
            </div>
            
             {/* Limit Indicators */}
             {isMaxLeft && <div className="absolute left-[-20px] top-1/2 -translate-y-1/2 text-red-500 font-bold text-xs animate-pulse">LOCK</div>}
             {isMaxRight && <div className="absolute right-[-20px] top-1/2 -translate-y-1/2 text-red-500 font-bold text-xs animate-pulse">LOCK</div>}
        </div>
        ...
        <div className="flex gap-4 w-full justify-center mt-2">
            <button 
                ...
                className="flex flex-col items-center justify-center w-12 h-12 bg-gray-800 hover:bg-gray-700 rounded-xl border border-gray-700 text-gray-400 hover:text-white active:bg-red-900/50 active:border-red-700 transition-all" 
                title="左打死 (Full Left)"
            >
                <ArrowLeftToLine size={18} />
                <span className="text-[10px] mt-0.5">左死</span>
            </button>
            <button 
                onClick={() => setTurns(0)} 
                className="flex flex-col items-center justify-center w-12 h-12 bg-gray-800 hover:bg-gray-700 rounded-xl border border-gray-700 text-gray-400 hover:text-white active:bg-blue-900/50 active:border-blue-700 transition-all" 
                title="回正 (Center)"
            >
                <Circle size={18} />
                <span className="text-[10px] mt-0.5">回正</span>
            </button>
            <button 
                onClick={() => setTurns(MAX_TURNS)} 
                className="flex flex-col items-center justify-center w-12 h-12 bg-gray-800 hover:bg-gray-700 rounded-xl border border-gray-700 text-gray-400 hover:text-white active:bg-red-900/50 active:border-red-700 transition-all" 
            ...
            >
                ...
                <span className="text-[10px] mt-0.5">右死</span>
            </button>
        </div>
